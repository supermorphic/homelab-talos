...
Run established-node lifecycle actions from `homelab-playbook`. These workflows share
one disruption Lease with retained disruptive Talos workflows and refuse to start while
another node is unavailable, cordoned, or in a recorded lifecycle state.
...
`talos reboot` cordons and drains workloads but keeps reusable Longhorn replicas for the
short expected outage. `talos maintenance-enter` also changes Longhorn scheduling and
...
the underlying fault, use `talos maintenance-exit` with the kind shown in the
