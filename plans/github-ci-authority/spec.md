...
and provide an operator runbook for creating, reading back, testing, and maintaining
the authoritative GitHub settings. The runbook is guidance, not desired state or a
...
29. As a repository administrator, I want an operator runbook for GitHub protections,
    so that I can create and verify the intended settings without treating repository
    files as authoritative configuration.
...
33. As a future maintainer, I want durable GitHub reconciliation deferred to a
    deliberate controller project, so that a static payload does not become a second
...
- Do not enable update restrictions, merge queue, required deployments, or required
  signed commits.
- GitHub's active repository settings and active ruleset are authoritative.
- Do not commit exported ruleset JSON, REST payloads, apply scripts, or recipes that
...
- Add an operator runbook that explicitly identifies itself as non-authoritative.
  It covers prerequisites, repository merge settings, ruleset creation through the
  GitHub UI or REST API, complete read-back, functional verification, periodic audit,
  and future-controller guidance.
...
- Apply and read back the GitHub protections before preparing the repository change,
  so that the production boundary is enforced throughout the transition.

## Testing Decisions

- Favor external behavior over implementation details. The primary seam is GitHub's
  effective repository and branch-rule state, not a checked-in representation of
  that state.
...
- Inspect the workflow structurally to confirm its triggers, lack of path filters,
  ...
  dependencies.
- Existing full-CI aggregation and repository hook behavior are the relevant prior
  art; no new testing framework or lower-level test seam is needed.
...
- Committing or generating a ruleset desired-state payload or executable apply
  mechanism.
...
- Repository merge settings were moved to squash-only immediately before this spec
  was published. The active `Protect main` ruleset had not yet been created.
...
- If durable automatically reconciled GitHub configuration is desired later, adopt
  Terraform or another controller as a separate architectural decision.
