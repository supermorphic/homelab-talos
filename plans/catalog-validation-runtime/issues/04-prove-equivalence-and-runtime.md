...
- [x] Standalone catalog validation, the complete test harness, and the full
      sequential validation contract each receive three comparable post-refactor
      runs with individual durations and medians.
- [x] The full sequential command passes all 32 suites in the unchanged order and
      retains the established fail-fast and reporting behavior.
- [x] Before and after medians are compared against the standalone target and
...
- [x] If any acceptable ceiling is exceeded, measured profiling identifies and
      reports the dominant remaining cost before any follow-up optimization is
      proposed.
- [x] Comparable Talos and Flux GitOps repositories are used only as contextual
      evidence, with differences in validation breadth called out explicitly.
- [x] The final report lists every command and outcome, any intentionally skipped
...
2026-07-31: `validation-report.md` records three-run before/after evidence,
equivalence, thresholds, external context, exclusions, and measured remaining
harness costs. Standalone and full stretch targets passed; the harness met its
acceptable ceiling and its missed target was profiled.
