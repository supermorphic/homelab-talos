#!/usr/bin/env bash
# Build a PATH containing exactly the commands samples.json declares the runtime
...
declared_runtime_commands() {
	local samples="$1"
	yq -r '.data."samples.json"' "$samples" |
		yq -r '.runtime.requiredCommands[]'
}

# Usage: runtime_sandbox_path <ConfigMap manifest> <sandbox-dir> [excluded-command...]
...
runtime_sandbox_path() {
	...
	((${#declared[@]} > 0)) || {
		echo 'samples.json declares no runtime commands' >&2
		return 1
	}
