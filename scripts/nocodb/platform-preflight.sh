...
render_job() {
  local metadata_body_md5 validator_body_md5
  ...
  validator_body_md5="$(awk '
    /^AS \$function\$/ { body = 1; print ""; next }
    /^\$function\$;/ { body = 0 }
    body { print }
  ' kubernetes/apps/automation-data/postgresql/app/scripts/domain-validation.sql |
    openssl dgst -md5 -r | awk '{print $1}')"
  [[ "$validator_body_md5" =~ ^[0-9a-f]{32}$ ]] || return 1
  # shellcheck disable=SC2016 # yq renders the fixed in-container shell and SQL literals.
  JOB_NAME="$job_name" RUN_ID="$run_marker" METADATA_BODY_MD5="$metadata_body_md5" VALIDATOR_BODY_MD5="$validator_body_md5" yq --null-input --output-format yaml '
    {
      "apiVersion": "batch/v1",
      "kind": "Job",
      "metadata": {
          ...
          "spec": {
            ...
            "containers": [{
              ...
              "env": [
                ...
                {"name": "NOCODB_VALIDATOR_BODY_MD5", "value": strenv(VALIDATOR_BODY_MD5)},
