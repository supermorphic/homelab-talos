...
reset_tree() {
	monitoring_fixture_reset "$template_root" "$tree_root" \
		kubernetes/apps/monitoring/alloy-logs/app/config.alloy \
		kubernetes/apps/monitoring/alloy-logs/app/values.yaml
}

run_validator() { (cd "$tree_root" && "$validator" alloy-logs) 2>&1; }
run_river_validator() { python "$river_validator" "$config" 2>&1; }
...
expect_pass() {
	...
	rg -Fq 'Alloy Logs source and render passed validation.' <<<"$output" || {
		echo "$description: scoped success message is missing." >&2
		echo "$output" >&2
		exit 1
	}
}
...
expect_river_pass() {
	...
	local output status
	set +e
	output="$(run_river_validator)"
	status="$?"
	set -e
	[[ "$status" -eq 0 ]] || {
		echo "$description: expected the Alloy River source to pass." >&2
		echo "$output" >&2
		exit 1
	}
}
...
set +e
unsupported_output="$(cd "$tree_root" && "$validator" unsupported 2>&1)"
unsupported_status="$?"
set -e
[[ "$unsupported_status" -eq 2 ]] || {
	echo "unsupported scope: expected exit 2, got $unsupported_status." >&2
...
expect_river_fail 'Pod annotation opt-out no longer drops targets' \
...
expect_river_fail 'Kubernetes source bypasses processing' \
...
replace_once "$config" \
	...
	'forward_to = [loki.write.default.receiver]'
expect_river_fail 'Talos source bypasses processing' \
...
replace_once "$config" \
	$'\tforward_to = [loki.write.default.receiver]\n}' \
	$'\tstage.static_labels {\n\t\tvalues = {\n\t\t\tpod_uid = "synthetic",\n\t\t}\n\t}\n\n\tforward_to = [loki.write.default.receiver]\n}'
expect_river_fail 'dynamic Kubernetes label added after allowlist' \
...
expect_river_fail 'Kubernetes final label allowlist expanded' \
...
expect_river_fail 'structured metadata enabled' \
...
replace_once "$config" \
	$'loki.write "default" {' \
	$'loki.write "default" {\n\twal {\n\t\tenabled = true\n\t}'
expect_river_fail 'Alloy WAL enabled' 'Refusing: Alloy Loki delivery must not enable a WAL.'
...
replace_once "$config" \
	...
	$'\t/*\n\trule {\n\t\tsource_labels = ["__meta_kubernetes_pod_annotation_observability_supermorphic_com_logs"]\n\t\taction        = "drop"\n\t\tregex         = `^disabled$`\n\t}\n\t*/'
expect_river_fail 'Pod annotation opt-out rule commented out' \
	'Refusing: Alloy Kubernetes Pod opt-out rule must drop only disabled targets.'
...
expect_river_fail 'comment-separated dynamic Kubernetes label added after allowlist' \
	'Refusing: Alloy Kubernetes processing stages must end with the exact label allowlist.'
...
replace_once "$config" \
	$'loki.write "default" {' \
	$'loki.write "default" {\n\twal /* validator gap */ {\n\t\tenabled = true\n\t}'
expect_river_fail 'comment-separated Alloy WAL enabled' \
...
expect_river_fail 'comment-separated bypass source added' \
...
expect_river_fail 'unterminated Alloy block comment added' \
...
expect_river_pass 'Alloy line comments with block-comment tokens'
...
expect_river_fail 'nonexistent Kubernetes Pod path capture' \
...
expect_river_fail 'comment-hidden invalid Kubernetes Pod path capture' \
	'Refusing: Alloy Kubernetes Pod path must use the complete UID/container capture.'

echo '23. Sending wildcard Pod paths directly to loki.source.file is rejected.'
reset_tree
configure_kubernetes_direct_flow
expect_river_fail 'Kubernetes source bypasses file discovery' \
...
replace_once "$config" \
	'targets    = local.file_match.kubernetes.targets' \
	'targets    = discovery.relabel.kubernetes_pods.output'
expect_river_fail 'Kubernetes source bypasses its configured file matcher' \
