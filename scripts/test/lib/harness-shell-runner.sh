...
run_registered_harness_shell_cases() (
	set -euo pipefail
	local result_fragment_root="${1:-}" jobs="${2:-}" start_index="${3:-}" harness_suite_id="${4:-}"
	validate_harness_shell_jobs "$jobs"
	[[ "$start_index" =~ ^[0-9]+$ ]] || {
		echo "Harness shell start index must be a non-negative integer: ${start_index:-<empty>}" >&2
		return 2
	}
	[[ "$harness_suite_id" =~ ^validation\.test-harness(-(core|observability|automation|ci-framework))?$ ]] || {
		echo "Invalid harness shell suite ID: ${harness_suite_id:-<empty>}" >&2
		return 2
	}
