...
def expect_acceptance(
    ...
    assert completed.stdout == "Test catalog passed validation: suites=131.\n"
...
def campaign_contract(root: Path, canonical: dict[str, Any]) -> None:
    assert all(
        not {"test.openbao-restore-drill", "test.openbao-issuance", "test.openbao-ha"}
        & set(campaign.get("members", []))
        for campaign in canonical["campaigns"].values()
    ), "OpenBao acceptance requires attended operator authority, never periodic execution."
    assert (
...
def execution_contract(root: Path, canonical: dict[str, Any]) -> None:
    ...
    expect_rejection(
        ...
        "Validation catalog/executions.ci count differs: catalog=46 ci=45.\n",
    )
    expect_rejection(
        root,
        canonical,
...
def access_boundary_contract(root: Path, canonical: dict[str, Any]) -> None:
    ...
    for campaign_name in ("verification", "scoped-verification"):
        ...
        assert (
            canonical["campaigns"][campaign_name]["members"].count("verification.nocodb") == 1
        ), f"active verification.nocodb must appear exactly once in {campaign_name}"
